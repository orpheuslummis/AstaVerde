describe("Vault Operations", () => {
    ...
    it("should deposit NFT to vault and receive 20 SCC", () => {
        ...
        cy.get("body").then(($body) => {
            const hasEligibleTokens =
                $body.find(".token-card").not(':contains("redeemed")').not(':contains("vaulted")').length > 0;
            ...
            cy.getSCCBalance().then((initialScc) => {
                ...
                cy.get(".token-card")
                    .not(':contains("redeemed")')
                    .not(':contains("vaulted")')
                    .first()
                    .within(() => {
                        // Click deposit button
                        cy.get("button")
                            .contains(/deposit|vault/i)
                            .click();
                    });
