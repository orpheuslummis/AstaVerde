...
import { config } from '../wagmi'; // Import the config
...
export function useContractInteraction(contractConfig: any, functionName: string) {
    ...
    const { data: simulationData, error: simulationError, refetch: refetchSimulation } = useSimulateContract({
        ...contractConfig,
        functionName,
    ...
    }, [publicClient, functionName, walletClient, writeContractAsync, contractConfig, isReadOnlyFunction]);
...
export function useBatchOperations(batchId: number, totalPrice: number) {
    ...
    }, [walletClient, publicClient, allowance, totalPrice, batchId, astaverdeContractConfig, getUsdcContractConfig, refetchAllowance, refetchBatches]);
