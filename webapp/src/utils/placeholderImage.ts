const generateFractalPath = (seed: number, iterations: number, angle: number): string => {
  const rules = ['F+F-F-F+F', 'F-F+F+F-F', 'F+F-F+F-F', 'F-F+F-F+F'];
  let path = 'F';
  ...
  let x = 150, y = 100;
  let dir = 0;
  const stepSize = 200 / (4 ** iterations);
  let svgPath = `M${x},${y}`;
...
const generatePlaceholderSVG = (id: number | string) => {
  ...
  const angle = (seed % 6 + 2) * Math.PI / 12;
  ...
  return `
    <svg xmlns="http://www.w3.org/2000/svg" width="300" height="200" viewBox="0 0 300 200">
      ...
      <path d="${fractalPath}" fill="none" stroke="rgba(255,255,255,0.5)" stroke-width="2" />
