...
- Extend `scripts/deploy-with-validation.js` (and any helpers) to accept Arbitrum network names (use `arbitrum-one` and `arbitrum-sepolia`) and load matching `.env.arbitrum-*` files.
...
- Provide Arbitrum-focused env examples (`webapp/.env.arbitrum` / `.env.arbitrum.example`) and set `NEXT_PUBLIC_CHAIN_SELECTION` to the Arbitrum key; wire `NEXT_PUBLIC_USDC_ADDRESS` to native USDC.
...
3) QA deploy to Arbitrum Sepolia via updated deploy script (fresh stack), save `deployments/arbitrum-sepolia` artifacts, and populate `webapp/.env.arbitrum` for testing with USDC: `0x75faf114eafb1BDbe2F0316DF893fd58CE46AA4d`.
...
- [x] `dev:sepolia` script repointed to Arbitrum Sepolia and `.env.arbitrum`.
- [x] `deploy:testnet` / `deploy:mainnet` default to Arbitrum networks; deploy script preflight supports Arbitrum RPC envs.
- [x] Added `webapp/.env.arbitrum.example` with native USDC addresses (421614 and mainnet note).
...
3. Deploy to Arbitrum Sepolia via `npm run deploy:testnet`; capture addresses/artifacts under `deployments/arbitrum-sepolia` and update `webapp/.env.arbitrum`.
