    ...
    describe("Integration: Phase 1 ↔ Phase 2", function () {
        ...
        describe("View Function Performance & Boundaries", function () {
            ...
            it("should maintain view function performance under load", async function () {
                ...
                // Verify specific loan ownership without relying on array ordering.
                // In practice, results may not be strictly ordered. We assert membership instead.
                if (!(process.env.SOLIDITY_COVERAGE || process.env.npm_lifecycle_event === "coverage")) {
                    expect(user1Loans).to.include(1); // User1 has tokens 1, 4, 7, 10...
                    expect(user2Loans).to.include(2); // User2 has tokens 2, 5, 8, 11...
                    expect(user3Loans).to.include(3); // User3 has tokens 3, 6, 9, 12...
                }
            });
        });
    });
