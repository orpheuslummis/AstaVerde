...
describe("AstaVerde Coverage Gaps", function () {
    ...
    describe("Gas Optimization - maxPriceUpdateIterations", function () {
        ...
        it("Should reject invalid iteration limits", async function () {
            // Test zero
            await expect(
                astaVerde.connect(owner).setMaxPriceUpdateIterations(0)
            ).to.be.revertedWith("Iteration limit must be between 1 and 1000");

            // Test above maximum
            await expect(
                astaVerde.connect(owner).setMaxPriceUpdateIterations(1001)
            ).to.be.revertedWith("Iteration limit must be between 1 and 1000");
        });

        it("Should reject non-owner setting iterations", async function () {
            await expect(
                astaVerde.connect(user1).setMaxPriceUpdateIterations(50)
        ...
        it("Should emit PriceUpdateIterationLimitReached when limit is hit", async function () {
            ...
            await expect(
                astaVerde.connect(owner).mintBatch([producer1.address], ["newbatch"])
        ...
        it("Should prevent DoS with bounded iterations", async function () {
            ...
            const tx = await astaVerde.connect(owner).mintBatch(
                [producer1.address],
                ["final"]
            );
    ...
    describe("Surplus USDC Recovery", function () {
        it("Should recover surplus USDC sent accidentally", async function () {
            ...
            await expect(
                astaVerde.connect(owner).recoverSurplusUSDC(owner.address)
        ...
        it("Should only allow owner to recover surplus", async function () {
            ...
            await expect(
                astaVerde.connect(user1).recoverSurplusUSDC(user1.address)
            ).to.be.revertedWithCustomError(astaVerde, "OwnableUnauthorizedAccount");
        });
        ...
        it("Should reject recovery to zero address", async function () {
            ...
            await expect(
                astaVerde.connect(owner).recoverSurplusUSDC(ethers.ZeroAddress)
    ...
    describe("Price Invariant Testing", function () {
        it("Should enforce basePrice >= priceFloor invariant", async function () {
            ...
            await expect(
                astaVerde.connect(owner).setBasePrice(currentFloor - 1n)
        ...
        it("Should enforce priceFloor <= basePrice invariant", async function () {
            ...
            await expect(
                astaVerde.connect(owner).setPriceFloor(currentBase + 1n)
        ...
        it("Should allow valid setPriceFloor adjustments", async function () {
            ...
            await expect(
                astaVerde.connect(owner).setPriceFloor(newFloor)
        ...
        it("Should allow valid setBasePrice adjustments", async function () {
            ...
            await expect(
                astaVerde.connect(owner).setBasePrice(newBase)
        ...
        it("Should reject non-owner price adjustments", async function () {
            await expect(
                astaVerde.connect(user1).setPriceFloor(30 * USDC_PRECISION)
            ).to.be.revertedWithCustomError(astaVerde, "OwnableUnauthorizedAccount");

            await expect(
                astaVerde.connect(user1).setBasePrice(300 * USDC_PRECISION)
            ).to.be.revertedWithCustomError(astaVerde, "OwnableUnauthorizedAccount");
        });
        ...
        it("Should handle edge case: setting floor and base to same value", async function () {
            ...
            await expect(
                astaVerde.connect(owner).setBasePrice(samePrice - 1)
            ).to.be.revertedWith("Base price must be at least price floor");
        });
    });
