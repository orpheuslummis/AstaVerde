...
import { AstaVerde, EcoStabilizer, StabilizedCarbonCoin, MockUSDC } from "../typechain-types";
...
describe("Pause Functionality Tests", function () {
    ...
    beforeEach(async function () {
        ...
        ecoStabilizer = await EcoStabilizer.deploy(await astaVerde.getAddress(), await scc.getAddress());

        // Grant minter role to vault
        await scc.grantRole(await scc.MINTER_ROLE(), await ecoStabilizer.getAddress());

        // Mint test NFTs
        await astaVerde.mintBatch([producer.address, producer.address], ["cid1", "cid2"]);
    ...
    describe("AstaVerde Pause Behavior", function () {
        ...
        it("Should block trading functions during pause", async function () {
            ...
            await expect(astaVerde.mintBatch([producer.address], ["cid3"])).to.be.revertedWithCustomError(
                astaVerde,
                "EnforcedPause",
            );

            await expect(astaVerde.connect(producer).redeemToken(1)).to.be.revertedWithCustomError(
                astaVerde,
                "EnforcedPause",
            );
        });
        ...
        it("Should block admin pricing setters during pause", async function () {
            await expect(astaVerde.setPlatformSharePercentage(10)).to.be.revertedWithCustomError(
                astaVerde,
                "EnforcedPause",
            );

            await expect(astaVerde.setPriceFloor(ethers.parseUnits("5", USDC_DECIMALS))).to.be.revertedWithCustomError(
                astaVerde,
                "EnforcedPause",
            );

            await expect(astaVerde.setBasePrice(ethers.parseUnits("20", USDC_DECIMALS))).to.be.revertedWithCustomError(
                astaVerde,
                "EnforcedPause",
            );

            await expect(astaVerde.setMaxBatchSize(50)).to.be.revertedWithCustomError(astaVerde, "EnforcedPause");

            await expect(astaVerde.setAuctionDayThresholds(60, 30)).to.be.revertedWithCustomError(
                astaVerde,
                "EnforcedPause",
            );

            await expect(astaVerde.setPriceDelta(ethers.parseUnits("2", USDC_DECIMALS))).to.be.revertedWithCustomError(
                astaVerde,
                "EnforcedPause",
            );
        ...
        it("Should allow emergency recovery functions during pause", async function () {
            ...
            await expect(astaVerde.recoverSurplusUSDC(owner.address)).to.not.be.reverted;
            ...
            await expect(astaVerde.claimPlatformFunds(owner.address)).to.not.be.reverted;
        });
        ...
        it("Should allow recoverERC20 during pause", async function () {
            ...
            await expect(astaVerde.recoverERC20(await otherToken.getAddress(), ethers.parseEther("100"), owner.address))
                .to.not.be.reverted;
        });
        ...
        it("Should allow producer fund claims during pause", async function () {
            ...
            await expect(astaVerde.connect(producer).claimProducerFunds()).to.not.be.reverted;
        });
        ...
        it("Should allow pause/unpause by owner only", async function () {
            ...
            await expect(astaVerde.connect(buyer).pause()).to.be.revertedWithCustomError(
                astaVerde,
            ...
            await expect(astaVerde.connect(buyer).unpause()).to.be.revertedWithCustomError(
                astaVerde,
                "OwnableUnauthorizedAccount",
            );
        });
        ...
        it("Should block ERC1155 transfers during pause", async function () {
            ...
            await expect(
                astaVerde.connect(buyer).safeTransferFrom(buyer.address, other.address, 1, 1, "0x"),
            ).to.be.revertedWithCustomError(astaVerde, "EnforcedPause");
        });
    });
    ...
    describe("EcoStabilizer Pause Behavior", function () {
        ...
        it("Should block deposits during pause", async function () {
            await expect(ecoStabilizer.connect(buyer).deposit(1)).to.be.revertedWithCustomError(
                ecoStabilizer,
                "EnforcedPause",
            );

            await expect(ecoStabilizer.connect(buyer).depositBatch([1, 2])).to.be.revertedWithCustomError(
                ecoStabilizer,
                "EnforcedPause",
            );
        });
        ...
        it("Should allow adminSweepNFT regardless of pause state", async function () {
            ...
            await astaVerde
                .connect(buyer)
                .safeTransferFrom(buyer.address, await ecoStabilizer.getAddress(), 2, 1, "0x");

            // Should work during pause (vault is paused from beforeEach)
            await expect(ecoStabilizer.adminSweepNFT(2, owner.address)).to.not.be.reverted;

            // Test with unpause too
            // Send token 1 to vault (buyer owns it from buyBatch)
            await astaVerde
                .connect(buyer)
                .safeTransferFrom(buyer.address, await ecoStabilizer.getAddress(), 1, 1, "0x");

            // Should also work when not paused
            await ecoStabilizer.unpause();
            await expect(ecoStabilizer.adminSweepNFT(1, owner.address)).to.not.be.reverted;
        });
    ...
    describe("Pause State Transitions", function () {
        it("Should maintain correct state through pause/unpause cycles", async function () {
            ...
            expect(await astaVerde.getProducerBalance(producer.address)).to.equal(initialProducerBalance);
            expect(await astaVerde.platformShareAccumulated()).to.equal(initialPlatformShare);

            // Unpause
            await astaVerde.unpause();

            // Should be able to continue operations
            await expect(astaVerde.mintBatch([producer.address], ["cid3"])).to.not.be.reverted;
        });
        ...
        it("Should emit pause events", async function () {
            await expect(astaVerde.pause()).to.emit(astaVerde, "Paused").withArgs(owner.address);

            await expect(astaVerde.unpause()).to.emit(astaVerde, "Unpaused").withArgs(owner.address);
        });
    });
});
