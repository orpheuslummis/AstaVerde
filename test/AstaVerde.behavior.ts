...
import { createNewAddresses, mintUSDC } from "./helpers";
...
export function shouldBehaveLikeAstaVerde(): void {
  ...
  it("should fail to set starting price with zero value", async function () {
    ...
    await expect(this.astaVerde.setBasePrice(zeroStartingPrice)).to.be.revertedWith("Invalid starting price");
  });
  ...
  it("should work to buy full batch", async function () {
    ...
    await mintUSDC(user, this.mockUSDC, 100000n);
    ...
    expect(platformBalance).to.equal((usdcAmount * BigInt(Math.floor(0.3 * 100))) / BigInt(100));
  });
  ...
  it("should work to buy full batch with a tricky base price", async function () {
    ...
    // Assert that the platform share is correct
    const platformBalance = await this.mockUSDC.balanceOf(this.astaVerde);
    const expectedPlatformShare = BigInt(Math.ceil(0.3 * Number(price))) * tokenAmount;
  ...
  it("should work to buy a partial batch", async function () {
    ...
    expect(await this.astaVerde.balanceOf(user.address, tokenIds[0])).to.equal(1);
    expect(await this.astaVerde.balanceOf(user.address, tokenIds[1])).to.equal(1);
    expect(await this.astaVerde.balanceOf(user.address, tokenIds[2])).to.equal(0); // This token was not bought
  });
  ...
  it("should fail to buy too many tokens of a partial batch", async function () {
    ...
    await expect(this.astaVerde.connect(user).buyBatch(batchID, usdcAmount, tokenAmount)).to.be.revertedWith(
  ...
  it("should work to buy the rest of tokens of a partial batch", async function () {
    ...
    await this.mockUSDC.connect(user).approve(this.astaVerde, usdcAmount);
    await this.astaVerde.connect(user).setApprovalForAll(this.astaVerde, true);
    let tx = await this.astaVerde.connect(user).buyBatch(batchID, usdcAmount, tokenAmount);
    let receipt = await tx.wait();
    let block = await ethers.provider.getBlock(receipt.blockNumber);
    let timestamp = block.timestamp;
    await expect(tx).to.emit(this.astaVerde, "PartialBatchSold").withArgs(batchID, timestamp, tokenAmount);

    tokenAmount = BigInt(3); // Buy the 3 remaining tokens
    usdcAmount = price * tokenAmount;
    await this.mockUSDC.connect(user).approve(this.astaVerde, usdcAmount);

    tx = await this.astaVerde.connect(user).buyBatch(batchID, usdcAmount, tokenAmount);
    receipt = await tx.wait();
    block = await ethers.provider.getBlock(receipt.blockNumber);
    timestamp = block.timestamp;
    await expect(tx).to.emit(this.astaVerde, "BatchSold").withArgs(batchID, timestamp, 5);
  });
  ...
  */
  ...
  it("should handle refund when the USDC amount is greater than the total cost", async function () {
    ...
    const tokenAmount = BigInt(cids.length);
    const usdcAmount = price * tokenAmount;
    const overPaidAmount = usdcAmount + 1000n; // Overpay by 1000
    const user = this.signers.others[0]; // Use a user to buy, not the admin
    const initialUserBalance = 1000000n;
    ...
    await this.astaVerde.connect(user).setApprovalForAll(this.astaVerde, true);
    await this.astaVerde.connect(user).buyBatch(batchID, overPaidAmount, tokenAmount);
    const userBalanceAfterPurchase = await this.mockUSDC.balanceOf(user.address);
  ...
  it("should fail to redeem tokens by a non-owner", async function () {
    ...
    await this.astaVerde.connect(user1).buyBatch(batchID, usdcAmount, tokenAmount);
  ...
  it("should fail to redeem tokens when the contract is paused", async function () {
    ...
    try {
      ...
      if (!error.message.includes("EnforcedPause()")) {
        throw new Error("Expected 'EnforcedPause()' to be included in the error message");
      }
    }
  });
  ...
  it("should allow the owner to claim platform funds", async function () {
    ...
    await this.astaVerde.connect(user1).buyBatch(batchID1, usdcAmount1, tokenAmount1);
    await this.astaVerde.connect(user2).buyBatch(batchID2, usdcAmount2, tokenAmount2);
    ...
    const ownerFundsAfter = await this.mockUSDC.balanceOf(this.signers.admin.address);
  ...
  it("should fail when a non-owner tries to claim platform funds", async function () {
    const nonOwner = this.signers.others[0];
    try {
      await this.astaVerde.connect(nonOwner).claimPlatformFunds(nonOwner.address);
    } catch (error) {
      if (!error.message.includes("OwnableUnauthorizedAccount")) {
        throw new Error("Expected 'OwnableUnauthorizedAccount' to be included in the error message");
      }
    }
  });
