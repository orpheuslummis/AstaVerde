...
- (Legacy) Local full stack: `npm run dev:local`
...
- Env files:
    - Local dev + testnet: `webapp/.env.local` (manual; copy from `webapp/.env.local.example`).
    - Production: set env vars in Vercel (do not commit env files).
...
- Webapp change:
    - Run `npm run dev:sepolia` and iterate in `webapp/`.
    - Lint/build: `cd webapp && npm run lint && npm run build`.

- Deploy:
    - Sepolia: `npm run deploy:testnet` → update `webapp/.env.local` → `npm run dev:sepolia`.
...
- Prefer `npm run dev:sepolia` for webapp testing.
