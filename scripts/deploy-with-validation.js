        ...
        try {
            ...
            if (artifact.abi && Array.isArray(artifact.abi)) {
                ...
                if (contractName === 'EcoStabilizer') {
                    const hasGetUserLoansIndexed = artifact.abi.some(item => 
                        item.type === 'function' && item.name === 'getUserLoansIndexed'
                    );
...
try {
    const deployCommand = network === 'localhost' 
...
if (network === 'localhost') {
    ...
    try {
        // Copy AstaVerde ABI
        const astaVerdeABI = JSON.parse(fs.readFileSync(
            path.join(__dirname, "../artifacts/contracts/AstaVerde.sol/AstaVerde.json"),
            'utf8'
        ));
        fs.writeFileSync(
        ...
        try {
            ...
            deploymentInfo.ecostabilizer = JSON.parse(fs.readFileSync("deployments/localhost/EcoStabilizer.json")).address;
        ...
        if (deploymentInfo.scc && deploymentInfo.ecostabilizer) {
            localDevConfig.StabilizedCarbonCoin = {
                ...
                abi: JSON.parse(fs.readFileSync(
                    path.join(__dirname, "../artifacts/contracts/StabilizedCarbonCoin.sol/StabilizedCarbonCoin.json"),
                    'utf8'
                )).abi
            };
