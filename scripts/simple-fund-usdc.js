...
async function main() {
    ...
    for (const account of accounts) {
        try {
            ...
            const slot = hre.ethers.solidityPackedKeccak256(["address", "uint256"], [account.address, 0]);

            // Set balance to 50000 USDC (50000 * 10^6)
            const value = hre.ethers.toBeHex(amount, 32);

            await hre.network.provider.send("hardhat_setStorageAt", [usdc.target, slot, value]);
