...
async function main() {
    ...
    const usdcDeployment = JSON.parse(fs.readFileSync(path.join(__dirname, "../deployments/localhost/MockUSDC.json")));
        ...
        if (lastBatchId < 2) {
            ...
            if (charlieBalance < 2n) {
                ...
                if (batch1Available >= Number(needed)) {
                    ...
                    console.log(
                        `   Charlie buying ${needed} NFTs from Batch 1 at ${ethers.formatUnits(charlieTotal, 6)} USDC total`,
                    );
