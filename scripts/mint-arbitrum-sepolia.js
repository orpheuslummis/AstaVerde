...
function loadAstaVerdeDeployment(cliAddress) {
    ...
    if (!fs.existsSync(deploymentPath)) {
        throw new Error(
            "AstaVerde address not found. Pass --address or add deployments/arbitrum-sepolia/AstaVerde.json",
        );
    }
...
function buildMetadata({ tokenId, namePrefix, producerAddress, externalUrlBase, imageUri }) {
    ...
    const description = "Verified carbon offset NFT for testnet QA. Metadata generated by automated minting script.";
...
async function main() {
    const argv = yargs(hideBin(process.argv))
        ...
        .option("cidPrefix", {
            ...
            type: "string",
            default: "QmArbTest",
            describe: "CID prefix for mock metadata",
        })
