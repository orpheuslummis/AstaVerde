...
class ProjectStatus {
    ...
    analyzeTickets() {
        ...
        const tickets = fs.readdirSync(this.ticketsDir).filter((f) => f.endsWith(".md"));
    ...
    analyzeContracts() {
        ...
        const contracts = fs.readdirSync(contractsDir).filter((f) => f.endsWith(".sol") && !f.startsWith("Mock"));
    ...
    checkFrontend() {
        ...
        if (fs.existsSync(componentsDir)) {
            this.stats.frontend.components = fs.readdirSync(componentsDir).filter((f) => f.endsWith(".tsx")).length;
        }

        if (fs.existsSync(hooksDir)) {
            this.stats.frontend.hooks = fs.readdirSync(hooksDir).filter((f) => f.endsWith(".ts")).length;
        }
    }
    ...
    calculateProgress() {
        ...
        // Overall progress
        progress.overall = Math.round(progress.testing * 0.3 + progress.frontend * 0.3 + progress.security * 0.4);
