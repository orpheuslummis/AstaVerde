...
async function main() {
    ...
    for (const account of accounts) {
        ...
        const slot = hre.ethers.keccak256(
            hre.ethers.AbiCoder.defaultAbiCoder().encode(["address", "uint256"], [account.address, 0]),
        );

        // Set balance to 50000 USDC
        const paddedAmount = hre.ethers.zeroPadValue(hre.ethers.toBeHex(amount), 32);

        await hre.network.provider.send("hardhat_setStorageAt", [usdcAddress, slot, paddedAmount]);
    ...
    await hre.network.provider.send("hardhat_setStorageAt", [usdcAddress, totalSupplySlot, paddedTotal]);
